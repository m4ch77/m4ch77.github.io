...
async function loadPosts() {
  ...
  for (const src of sources) {
    ...
    /* 손으로 쓴 글에서 본문에 "# " 를 쓰면 h1 이 두 개가 됩니다
       (글 제목이 이미 h1 입니다). Notion 글은 자동으로 한 단계 낮추지만
       여기는 손으로 쓰는 곳이라 알려만 줍니다.

       코드 펜스는 빼고 봅니다. 셸 예제의 "# 주석" 이 걸리면 안 됩니다. */
    if (src.origin === "손") {
      const noCode = content.replace(
        /^[ \t]*(`{3,}|~{3,})[\s\S]*?^[ \t]*\1[ \t]*$/gm,
        "",
      );
      if (/^[ \t]{0,3}#(?!#)\s/m.test(noCode)) {
        say(
          `  ! ${src.label} — 본문에 "# " 제목이 있습니다. 글 제목이 이미 h1 이라` +
            ` h1 이 두 개가 됩니다. "## " 부터 쓰세요.`,
        );
      }
    }
